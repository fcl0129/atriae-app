...
export async function POST(request: Request) {
  try {
    ...
    for (const run of dueRuns) {
      const claimed = await repository.claimRunForDelivery(run.id, run.status, nowIso);
      if (!claimed) {
        alreadyClaimed += 1;
        continue;
      }
      ...
      await deliverRun(repository, claimed);
      const updated = await repository.getRunById(claimed.id);
      if (!updated) continue;
    ...
    console.error("[digests][cron][failed]", {
