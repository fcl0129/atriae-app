        ...
        <div className="grid gap-4 md:grid-cols-2 md:gap-5">
          {areas.map((area, index) => (
            <Reveal key={area.title} delay={index * 70}>
              <article className="surface-paper h-full p-6 md:p-7">
                <h3 className="font-serif text-[30px] leading-[1.06] text-foreground">{area.title}</h3>
                <p className="pt-3 text-[16px] leading-[1.65] text-foreground/82">{area.description}</p>
                <Link
                  href={area.href}
                  className="mt-5 inline-flex text-[0.68rem] font-medium uppercase tracking-[0.18em] text-muted-foreground transition-colors duration-300 hover:text-foreground"
                >
                  Learn more
                </Link>
              </article>
